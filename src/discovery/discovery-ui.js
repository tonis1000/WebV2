...
function ensureUi(){
  ...
  if (controls && !controls.children.length) {
    ...
    const hint=document.createElement('span');hint.className='muted small';hint.textContent='GitHub applies this to repository pushed_at. Recent Web sends the same window to Brave. Curated and STRM feeds are live-checked but do not fabricate per-entry age.';controls.appendChild(hint);
  }
  if (button.dataset.bound!=='1') {
    button.dataset.bound='1';button.addEventListener('click',openPanel);$('discovery-close')?.addEventListener('click',closePanel);$('discovery-scan-local')?.addEventListener('click',scanLocalSources);$('discovery-scan-curated')?.addEventListener('click',()=>scanExternalProvider(CURATED_REMOTE_FEEDS_PROVIDER));$('discovery-scan-github')?.addEventListener('click',()=>scanExternalProvider(GITHUB_PUBLIC_PLAYLISTS_PROVIDER));$('discovery-scan-web')?.addEventListener('click',()=>scanExternalProvider(RECENT_WEB_SEARCH_PROVIDER));$('discovery-scan-strm')?.addEventListener('click',()=>scanExternalProvider(STRM_SPECIFIC_DISCOVERY_PROVIDER));$('discovery-cancel-external')?.addEventListener('click',cancelExternalDiscovery);$('discovery-verify-all')?.addEventListener('click',verifyAll);$('discovery-cancel-verify')?.addEventListener('click',cancelVerification);
  }
  return {button,panel};
}
...
function render(){
  ...
  for(const id of ['discovery-scan-local','discovery-scan-curated','discovery-scan-github','discovery-scan-web','discovery-scan-strm']){const control=$(id);if(control)control.disabled=disabled;}
