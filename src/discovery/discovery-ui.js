...
import { discoverCuratedRemoteFeeds, discoverGithubPublicPlaylists, discoverRecentWebSearch, CURATED_REMOTE_FEEDS_PROVIDER, GITHUB_PUBLIC_PLAYLISTS_PROVIDER, RECENT_WEB_SEARCH_PROVIDER } from './external-discovery-client.js';
import { verifyCandidates, verifyWithConcurrency } from './verifier-client.js';

const BUILD_ID='20260926-discovery-phase4-recent-web-search';
...
function ensureUi(){
  ...
  if (controls && !controls.children.length) {
    ...
    const hint=document.createElement('span');hint.className='muted small';hint.textContent='GitHub applies this to repository pushed_at. Recent Web sends the same window to Brave. Curated feeds are live-checked but may not expose per-entry age.';controls.appendChild(hint);
  }
  if (button.dataset.bound!=='1') {
    button.dataset.bound='1';button.addEventListener('click',openPanel);$('discovery-close')?.addEventListener('click',closePanel);$('discovery-scan-local')?.addEventListener('click',scanLocalSources);$('discovery-scan-curated')?.addEventListener('click',()=>scanExternalProvider(CURATED_REMOTE_FEEDS_PROVIDER));$('discovery-scan-github')?.addEventListener('click',()=>scanExternalProvider(GITHUB_PUBLIC_PLAYLISTS_PROVIDER));$('discovery-scan-web')?.addEventListener('click',()=>scanExternalProvider(RECENT_WEB_SEARCH_PROVIDER));$('discovery-cancel-external')?.addEventListener('click',cancelExternalDiscovery);$('discovery-verify-all')?.addEventListener('click',verifyAll);$('discovery-cancel-verify')?.addEventListener('click',cancelVerification);
  }
  return {button,panel};
}
...
function renderLanes(snapshot){
  ...
  const rows=[['My Playlist',snapshot.lanes.myPlaylist],['Saved Playlists',snapshot.lanes.savedPlaylists],['Xtream loaded',snapshot.lanes.xtream],['Curated feeds',snapshot.lanes.curatedRemoteFeeds],['GitHub recent',snapshot.lanes.githubPublicPlaylists],['Recent web',snapshot.lanes.recentWebSearch],['Unique',snapshot.lanes.total]];
...
function render(){
  ...
  for(const id of ['discovery-scan-local','discovery-scan-curated','discovery-scan-github','discovery-scan-web']){const control=$(id);if(control)control.disabled=disabled;}
  ...
  for (const raw of snapshot.candidates) {
    ...
    if(item.startupMs!==null||item.lastHttpStatus!==null||item.mediaType||item.verificationDetail){const detail=document.createElement('span');detail.textContent=[item.startupMs!==null?`${item.startupMs} ms`:'',item.lastHttpStatus!==null?`HTTP ${item.lastHttpStatus}`:'',item.mediaType||'',item.verificationDetail||''].filter(Boolean).join(' · ');card.appendChild(detail);}const actions=document.createElement('div');actions.className='discovery-card-actions';const verify=document.createElement('button');verify.type='button';verify.className='button ghost mini';verify.textContent='Verify';verify.disabled=verifyBusy||externalBusy||localBusy;verify.addEventListener('click',()=>verifyOne(item.candidateId));actions.appendChild(verify);card.appendChild(actions);results.appendChild(card);
  }
}
...
async function scanLocalSources(){
  ...
  try{const context=await readLocalSourceContext();const result=collectLocalCandidates(selected,context);const notes=[];if (!context.myPlaylistAvailable) notes.push('My Playlist snapshot skipped while temporary catalog is active');if (!context.loadedXtream) notes.push('Xtream lane uses only an account already loaded in Playlist Manager');const base=`Local scan complete · ${result.candidates.length} unique local candidate${result.candidates.length===1?'':'s'}`;state.setScanResult({candidates:result.candidates,lanes:result.lanes,message:notes.length?`${base} · ${notes.join(' · ')}`:base});} catch (error) {state.setScanError(error?.message||String(error));}render();
}
...
async function scanExternalProvider(provider){
  ...
  const meta=provider===GITHUB_PUBLIC_PLAYLISTS_PROVIDER?{label:'GitHub Public Playlists',lane:'githubPublicPlaylists',run:discoverGithubPublicPlaylists}:provider===RECENT_WEB_SEARCH_PROVIDER?{label:'Recent Web Search',lane:'recentWebSearch',run:discoverRecentWebSearch}:{label:'Curated Remote Feeds',lane:'curatedRemoteFeeds',run:discoverCuratedRemoteFeeds};
  externalController=new AbortController();state.setExternalScanning(`Searching ${meta.label} · requested window ${freshness}…`);render();
  try{const result=await meta.run(selected,{freshness,signal:externalController.signal});const note=result.freshnessApplied?`freshness applied · ${result.freshnessRequested}`:'live feed check; per-entry age unavailable';state.mergeExternalResult({provider,lane:meta.lane,candidates:result.candidates,count:result.candidates.length,message:`${meta.label} complete · ${result.candidates.length} candidate${result.candidates.length===1?'':'s'} · ${note}`});}
...
ensureUi();
const api=Object.freeze({buildId:BUILD_ID,open:openPanel,close:closePanel,scanLocal:scanLocalSources,scanExternal:scanExternalSources,scanGithub:scanGithubSources,scanRecentWeb:scanRecentWebSources,verifyOne,verifyAll,cancelExternalDiscovery,cancelVerification,snapshot:()=>state.snapshot()});
window.WebTVDiscovery=api;
window.WebTVDiscoveryPhase1=api;
console.info(`[WebTV] Discovery Phase 4 loaded · ${BUILD_ID} · curated + GitHub + recent web + separate verifier · no save`);
