...
function render(){
  ...
  for(const id of ['discovery-scan-local','discovery-scan-curated','discovery-scan-github','discovery-scan-web','discovery-scan-strm','discovery-scan-official']){const control=$(id);if(control)control.disabled=disabled;}
  ...
  for (const raw of snapshot.candidates) {
    ...
    if(item.startupMs!==null||item.lastHttpStatus!==null||item.mediaType||item.verificationDetail){const detail=document.createElement('span');detail.textContent=[item.startupMs!==null?`${item.startupMs} ms`:'',item.lastHttpStatus!==null?`HTTP ${item.lastHttpStatus}`:'',item.mediaType||'',item.verificationDetail||''].filter(Boolean).join(' · ');card.appendChild(detail);}const actions=document.createElement('div');actions.className='discovery-card-actions';const verify=document.createElement('button');verify.type='button';verify.className='button ghost mini';const verifiable=!['official-page','official-embed'].includes(item.candidateKind);verify.textContent=verifiable?'Verify':'Official fallback';verify.disabled=verifyBusy||externalBusy||localBusy||!verifiable;verify.addEventListener('click',()=>verifyOne(item.candidateId));actions.appendChild(verify);card.appendChild(actions);results.appendChild(card);
  }
}
