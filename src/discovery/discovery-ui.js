...
const BUILD_ID='20260926-discovery-phase46-authorized-xtream';
...
function ensureUi(){
  ...
  if (controls && !controls.children.length) {
    ...
    const hint=document.createElement('span');hint.className='muted small';hint.textContent='GitHub applies this to repository pushed_at. Recent Web sends the same window to Brave. Curated, STRM, Official and Authorized Xtream are live checks and do not fabricate per-entry age.';controls.appendChild(hint);
  }
...
function render(){
  ...
  for(const id of ['discovery-scan-local','discovery-scan-curated','discovery-scan-github','discovery-scan-web','discovery-scan-strm','discovery-scan-official','discovery-scan-xtream']){const control=$(id);if(control)control.disabled=disabled;}
