...
import { promoteCandidate, keepXtreamAccount } from './promotion.js';

const BUILD_ID='20260926-discovery-phase51-explicit-promote';
...
function render(){
  ...
  for (const raw of snapshot.candidates) {
    ...
    if(verifiable&&item.verificationStatus==='VERIFIED'&&item.saveEligible!==false){const add=document.createElement('button');add.type='button';add.className='button mini discovery-promote-source';add.textContent=Object.keys(item.requiredHeaders||{}).length?'Headers not persistable':'Add this source';add.disabled=verifyBusy||externalBusy||localBusy||promotionBusy.has(item.candidateId)||Object.keys(item.requiredHeaders||{}).length>0;add.addEventListener('click',()=>promoteOne(item.candidateId));actions.appendChild(add);}
...
console.info(`[WebTV] Discovery Phase 5.1 loaded · ${BUILD_ID} · explicit promote only · Xtream credentials stay server-side`);
