...
export const PROVIDER_FLAGS=Object.freeze({[CURATED_REMOTE_FEEDS_PROVIDER]:true});
...
export async function discoverCuratedRemoteFeeds(channel,{freshness='7d',endpoint=DISCOVERY_ENDPOINT,fetchImpl=fetch,signal,timeoutMs=EXTERNAL_DISCOVERY_TIMEOUT_MS}={}){
  if(!PROVIDER_FLAGS[CURATED_REMOTE_FEEDS_PROVIDER])return {provider:CURATED_REMOTE_FEEDS_PROVIDER,disabled:true,candidates:[],reports:[]};
  ...
  try{
    const response=await fetchImpl(`${String(endpoint).replace(/\/$/,'')}/discover`,{
      ...
      body:JSON.stringify({provider:CURATED_REMOTE_FEEDS_PROVIDER,freshness,channel:{id:String(channel.id||''),originalId:String(channel.originalId||''),name:String(channel.name||''),tvgId:String(channel.tvgId||'')}}),
    });
