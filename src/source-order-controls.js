...
let cache=[],cacheAt=0;
async function myChannels(force=false){
  const now=Date.now();
  if(!force&&cache.length&&now-cacheAt<5000)return cache;
  try{cache=await window.WebTVMyPlaylistAPI?.getMyPlaylist?.()||[];cacheAt=now;}catch{}
  return cache;
}
...
function routeBelongsToSource(routeKey,source){
  ...
  try{const u=new URL(routeKey);const nested=u.searchParams.get('url');if(nested===source)return true;if(nested){try{if(decodeURIComponent(nested)===source)return true;}catch{}}}catch{}
...
document.addEventListener('click',e=>{
  const t=e.target;
  if(t?.id==='source-editor-save'){
    resolveEditorChannel().then(channel=>{
      if(!channel)return;
  ...
  if(btn?.textContent?.trim()==='Sources'){
    setTimeout(()=>ensureEditorControls().catch(()=>{}),60);
    return;
  }
  if(t?.id==='diagnostics-toggle'||btn?.id==='diagnostics-toggle')setTimeout(refreshDiagnosticsMode,0);
},true);

window.addEventListener('webtv:source-order-mode',refreshDiagnosticsMode);
function start(){refreshDiagnosticsMode();console.info('[WebTV] Source order controls ready · event-driven · AUTO Health + MANUAL user order + per-channel health reset');}
if(window.WebTVPlaylistAPI?.ready)start();else window.addEventListener('webtv:ready',start,{once:true});
window.WebTVSourceOrder={getMode,setMode,resetHealthForChannel};
console.info('[WebTV] Source order controls loaded · event-driven; no global MutationObserver');
