...
async function requireAdmin(request, env, origin) {
  ...
  try {
    ...
    const response = env.REGISTRY?.fetch
      ? await env.REGISTRY.fetch(new Request('https://webtv-registry.internal/api/session/validate', requestInit))
      : await fetch(`${registry.replace(/\/+$/, '')}/api/session/validate`, requestInit);
...
async function providerJson(server, username, password, action = '') {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const safeTarget = `${url.origin}${url.pathname}${action ? `?action=${encodeURIComponent(action)}` : ''}`;
      throw new Error(`Xtream provider HTTP ${response.status} · ${safeTarget}${preview ? ` · ${preview}` : ''}`);
    }
