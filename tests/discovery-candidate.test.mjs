...
assert.equal(state.snapshot().candidates.length,3);
assert.equal(state.snapshot().candidates.every(item=>item.verified===false),true);
assert.equal(state.snapshot().candidates.every(item=>item.discoveryProvider==='local-shell'),true);
