...
assert.equal(status.version,'1.2');
...
try{
  ...
  const response=await discovery.fetch(request,{});assert.equal(response.status,200);const body=await response.json();assert.equal(body.provider,'curated-remote-feeds');assert.equal(body.freshnessRequested,'24h');assert.equal(body.freshnessApplied,false);assert.equal(body.candidates.length,1,'dedupe should collapse same MEGA URL from all curated feeds');assert.equal(body.candidates[0].sourceUrl,'https://good.test/mega.m3u8');assert.ok(maxInFlight<=2,`expected max concurrency <=2, saw ${maxInFlight}`);
